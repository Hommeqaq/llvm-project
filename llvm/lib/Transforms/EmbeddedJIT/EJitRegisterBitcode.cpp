...
static std::string extractAndSerialize(Module &M,
  ...
  for (GlobalVariable &GV : Extracted->globals()) {
    ...
    // Globals keep dso_local: data access stays direct ADRP+ADD (Small code
    // model, ±4GB reaches the JIT slab), matching both the AOT form and the
    // runtime policy measured on DlschCcScheduler (EJitOrcEngine.cpp:
    // "Globals: keep dso_local ... So globals are NOT cleared") — clearing it
    // would force a per-global dependent GOT load for no reach benefit.
    // Unlike the closure-helper functions above (BL's ±128MB reach requires
    // JITLink PLT stubs, hence their setDSOLocal(false)), globals are
    // deliberately left as-is. The bit persists on its own: internal linkage
    // set it via setLinkage's isImplicitDSOLocal, and the external conversion
    // above (default visibility + external linkage) does not clear it. A pool
    // placed beyond ADRP's ±4GB fails explicitly at JIT link time
    // (pre-existing baseline boundary, unchanged by externalization).
