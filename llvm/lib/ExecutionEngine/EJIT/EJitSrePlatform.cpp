...
// (EJIT_FIXED_CODE_POOL_IN_CODESEG); the platform MUST supply the strong
...
#if defined(EJIT_FIXED_CODE_POOL_IN_CODESEG)
extern "C" unsigned
...
// Fixed code-pool region bounds, defined by the linker script (a .data.ejit
...
extern const unsigned char __data_jit_start[] __attribute__((weak));
extern const unsigned char __data_jit_end[] __attribute__((weak));
#else
extern const unsigned char __data_jit_start[];
extern const unsigned char __data_jit_end[];
#endif
}
#endif
...
llvm::ejit::makeSreCodePoolManager() {
  ...
  // The linker script reserves [.data.ejit] = [__data_jit_start, __data_jit_end)
  ...
  // [__data_jit_start, AlignedBase) are wasted alignment slack (at most ~2MiB),
  ...
  {
    uintptr_t FBase = reinterpret_cast<uintptr_t>(__data_jit_start);
    uintptr_t FEnd = reinterpret_cast<uintptr_t>(__data_jit_end);
...
#ifdef EJIT_FIXED_CODE_POOL_IN_CODESEG
