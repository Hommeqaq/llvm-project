; -ejit-inline-cache (ON): emits a jit_icache probe (ejit_icache_try[_Nd])
; before the taskpool compile_or_get call; the hit path (jit_icache_dispatch)
; calls the cached specialization directly with NO ejit_taskpool_release_read.
; Default (OFF): the original 4-block wrapper, no icache.
; Idempotent: running the pass twice does not duplicate the icache blocks.
...
; --- 1D entry (<=2 dims): icache probe uses the fixed 1D fast path. ---
; ICACHE-LABEL: define i32 @one_dim_entry(
; ICACHE: call i32 @ejit_icache_try_1d(i32 {{.*}}, i32 {{.*}}, i32 {{.*}}, ptr {{.*}})
; ICACHE-LABEL: jit_icache_dispatch:
; ICACHE-NOT: call void @ejit_taskpool_release_read
; ICACHE: ret

; --- 3D entry (>2 dims): icache probe uses the generic entry. ---
; ICACHE-LABEL: define i32 @three_dim_entry(
; ICACHE: call i32 @ejit_icache_try(i32 {{.*}}, ptr {{.*}}, i32 3, ptr {{.*}})
...
; --- Idempotent: two passes emit the 1D probe exactly once. ---
; IDEM-LABEL: define i32 @one_dim_entry(
; IDEM-COUNT-1: call i32 @ejit_icache_try_1d(i32 {{.*}}, i32 {{.*}}, i32 {{.*}}, ptr {{.*}})
