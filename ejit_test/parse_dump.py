...
The script auto-detects objdump: tries --objdump, then $EJIT_OBJDUMP, then
build_release_x86/bin/llvm-objdump, then aarch64*-linux-gnu-objdump.
"""
...
HEX_LINE_RE = re.compile(
    r"^\s*[0-9a-fA-F]{8}:\s+([0-9a-fA-F]{2})\s+([0-9a-fA-F]{2})\s+([0-9a-fA-F]{2})\s+([0-9a-fA-F]{2})\s*$"
)
HEX_HEADER_RE = re.compile(
    r"post-link code hex dump\s+name=(\S+)\s+size=(\d+)"
)
...
def find_objdump(explicit=None):
    ...
    # LLVM objdump from build
    for build_dir in ["build_release_x86", "build_debug_x86"]:
        p = Path(f"../../{build_dir}/bin/llvm-objdump")
        if p.exists():
            candidates.append(str(p))
        p2 = Path(f"../{build_dir}/bin/llvm-objdump")
        if p2.exists():
    ...
    try:
        ...
        if is_llvm:
            cmd = [
                ...
                "-b",
                "binary",
                "--no-show-raw-insn" if False else "-d",
...
def main():
    ...
    if not objdump:
        print(
            "Error: no objdump found. Use --objdump or set $EJIT_OBJDUMP.\n"
            "Tried: llvm-objdump, aarch64*-linux-gnu-objdump",
            file=sys.stderr,
        )
        sys.exit(1)
