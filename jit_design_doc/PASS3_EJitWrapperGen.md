...
命中路径 = 探针（GEP 移位索引 + load + null-check）+ 尾调用 `br`；无栈帧、无 call、无 `release_read`、无跨核 RMW。对比 icache off / 首次 miss 的 taskpool 路径（`compile_or_get` + `readers_` RMW + 桶扫描 + `release_read`），命中路径压到 4–9 条指令。
