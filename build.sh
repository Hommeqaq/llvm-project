  ...
  if [ "$type" = "debug" ]; then
    if [ "$variant" = "static" ]; then
      ...
      cmake -S "${LLVM_SRC}" -B "${build_dir}" \
        ...
        -DEJIT_COMPILE_TIMING_ENABLE=${EJIT_COMPILE_TIMING_ENABLE} \
...
EJIT_COMPILE_TIMING_ENABLE=OFF
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --compile-timing) EJIT_COMPILE_TIMING_ENABLE=ON ;;
    --no-compile-timing) EJIT_COMPILE_TIMING_ENABLE=OFF ;;
    -h|--help)
      sed -n '2,31p' "$0"
      exit 0
      ;;
